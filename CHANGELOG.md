...
남은 것: 운동량과 충격량(3), 에너지 띠와 반도체(3), 파동의 진동과 굴절(4), 빛의 이중성(4)은 오개념이 5개 미만이라 진단검사가 3~4문항으로 줄고 L1 학습량도 짧다. 오개념 보강이 필요하다.
...
| 파일 | 내용 |
|---|---|
| `public/js/bkt.js` | 신규. 이해도 계산 순수 모듈 |
| `public/firebase/firestore.js` | knowledgeState CRUD, 관측 반영, 레벨 대상 선정, 승급 판정, 이해도 리셋 |
| `public/js/feedback.js` | 승급 판정 교체, 재도전 제외, 진행도 표시 |
| `public/js/app.js` | 순환 출제 헬퍼, 단원 정보 모달, 나가기 목적지 |
| `public/js/keyword.js` | 진단 오개념 누적, 순환 출제 연결 |
| `public/js/mypage.js` | 진단 풀 사용, 순환 출제 연결 |
| `public/js/quiz.js` | 문항 태그를 채점 결과에 전달 |
| `functions/index.js` | 문항 오개념 태깅, 우선 겨냥 지시, 사진 진단 정리 |
| `functions/seed.js` | 열역학·특수 상대성 오개념 16개 + 판별 문장 48개 |
| `public/index.html` | 단원 정보 모달, 문제 화면 상단 버튼 |
| `public/css/base.css`, `feedback.css` | 정보 버튼, 진행도 막대, 텍스트 버튼 |

### 7-11. 남은 것

- 사전 진단검사 화면(설계 단계 8) — 소단원 첫 진입 시 판별 문장 5개로 초기 이해도 설정
- 마이페이지 이해도 시각화(단계 9), "교정된 오개념" 통계 복원
- 파라미터 민감도 분석(단계 10, 논문 실험)
- 기존 오개념 17개에 판별 문장이 없음. 단계 8이 문장을 표본으로 쓰므로 그 전에 보강 필요
