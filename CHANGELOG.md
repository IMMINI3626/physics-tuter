...
### 7-9-2. 판별 문장 전수 검증 (단계 8-1)

배포 후 실제 문제에서 오류가 발견됐다. "N극이 위를 향하도록 자석을 가까이 할 때와 S극이 위를 향하도록 멀리할 때 유도 전류 방향이 같다"는 **옳은 문장**인데(자석을 뒤집고 운동도 뒤집으면 부호가 두 번 바뀌어 원래대로 돌아온다) AI가 틀린 문장으로 라벨했고, 옳게 판단해 체크하지 않은 학생이 오답 처리됐다. 해설도 자기 모순이었다.

**뿌리는 구조에 있다.** 서버는 문장이 5개인지, `isWrong`이 불리언인지, 태그가 목록에 있는 id인지만 검사한다. 문장이 사실인지는 아무도 검사하지 않는다. 정답이 무엇인지를 AI가 정하고 그게 곧 채점 기준이 된다.

대책은 Level 1 문장을 검증된 판별 문장 풀에서 가져오는 것인데, 그러려면 그 풀부터 믿을 수 있어야 한다. 343개를 전수 검토해 **15개에서 문제를 찾았다**.

| 유형 | 예 | 개수 |
|---|---|---|
| 라벨이 사실과 다름 | "2배 먼 거리를 이동했으므로 운동에너지도 2배" — 일정한 힘이면 참(W=Fd) | 5 |
| 물리 문장이 아님 | "진자의 현재 위치만 보고 판단할 뿐 처음 높이는 고려하지 않는다" — 논문의 학생 응답 서술을 그대로 옮김 | 4 |
| 조건이 빠져 애매 | "힘이 사라지면 물체는 곧 멈춘다" — 마찰이 있으면 참 | 6 |

데이터 자체가 모순인 것도 둘 있었다. WR2는 오개념의 `맞는개념` 필드가 "맞으나"라고 스스로 참이라고 썼고, ME12-S01은 같은 단원의 다른 두 문장과 정반대였다.

**조치.** 문장 10개 수정, 오개념 9개 폐기(WR2·AT6은 오류, AT4·AT5·WI1~WI5는 고교 교육과정 밖 — 오비탈·확률밀도는 화학Ⅱ, 음압 진폭은 고교 물리에 없음), 오개념 7개 신설.

폐기하면 원자 모형과 파동의 간섭이 3개까지 떨어져 진단검사 5문항이 불가능해지므로 같은 변경에서 채웠다. 이 과정에서 **"원자 모형과 전기력" 소단원에 전기력 오개념이 하나도 없던 것**이 드러나 전기력 개념 영역을 새로 만들었다. 근거는 네 편 모두 서지를 확인했다(Ivanjek 외 2015, Maloney 외 2001 CSEM, Maloney 1985, Wosilait 외 1999).

**시딩이 삭제를 안 하고 있었다.** `batchUpload`가 덮어쓰기만 해서, seed에서 지운 항목이 Firestore에 그대로 남았다. 폐기한 오개념으로 계속 문제가 나올 수 있었다. 업로드 전에 seed에 없는 문서를 지우는 단계를 넣었다(정적 데이터만, 사용자 데이터는 건드리지 않는다).

**결과.** 오개념 130 → 128, 판별 문장 343 → 342, 개념 영역 19 → 20. 14개 소단원 모두 5개 이상을 유지하고, 모든 오개념이 틀린 문장과 옳은 문장을 함께 갖는다.

### 7-9-3. 채점·이해도 경로 결함 수정 (단계 8-2)

한 세트 5문장 중 **2개의 참·거짓 라벨이 반대**로 나왔다. 학생은 둘 다 물리적으로 옳게 판단했는데 하나는 "옳은 문장을 잘못 골랐다"로 감점, 하나는 "틀린 문장을 못 찾았다"로 처리돼 35점을 받았다. 조사하면서 결함 6개를 확인했다.

**4분류 로직 자체는 정상이었다.** `isWrong`(문장이 틀렸나) × 체크 여부로 나누는 분기는 논리적으로 맞다. 문제는 그 입력값이 오염됐고, 학생이 손대지도 않은 문항이 집계에 섞인 것이었다.

**문항 검수를 넣었다.** 채점할 때 AI에게 "학생 답변과 무관하게 이 문장 자체가 거짓인가"를 다시 판정하게 하고, 문제를 만들 때의 판정과 어긋나면 그 문항을 무효로 만든다. 실제로 이번 사례에서 AI 해설은 이미 정답을 알고 있었다 — "이 문장은 틀린 진술입니다"라고 써놓고 라벨만 반대였다. 두 판정을 대조하기만 해도 잡힌다.

무효 문항은 점수에서 빼고(남은 문항 기준으로 만점 환산), 이해도 관측에서 제외하고, 화면에는 "검수 중인 문항"으로 따로 보여준다. 문항 오류의 책임을 학생에게 지울 수는 없다. 로그에 `isVoided`로 남겨 오류율도 집계한다.

**안 푼 문제로 이해도가 오르고 있었다.** 이게 제일 컸다. 미체크 문항의 `isCorrectAnswer`를 AI의 임의 판단으로 채우고 있어서, 학생이 손도 안 댄 문장이 "이해했다"는 관측으로 들어갔다. 이제 서버가 확정한다.
...
못 찾은 것을 제외하지 않고 오답으로 세는 이유는, 그게 "그 오개념을 알아보지 못했다"는 진짜 증거이기 때문이다. 제외하면 아무것도 체크 안 할 때 이해도가 영원히 안 떨어지는 구멍이 생긴다.

**부제도 고쳤다.** "틀린 문장 2개 중 1개 정답"인데 정작 정답 칸은 비어 있었다. 미체크 문항까지 정답으로 세고 있었다.

**분류 이름은 그대로 뒀다.** 풀어 쓰는 안을 검토했지만 기존 이름이 더 낫다고 판단했다. 카드 안의 "틀린 이유" 라벨만 "이 문장이 틀린 이유"로 바꿔 해설 본문과 어긋나 보이던 것을 없앴다.

**이해도를 초기화해야 한다.** 지금까지 쌓인 값은 오염된 관측으로 만들어졌다. `functions/reset-knowledge.js`를 추가했다(기본은 미리보기, `--apply`로 실제 삭제). 이해도와 진단검사 완료 표시만 지우고 학습 기록·레벨은 남긴다.

### 7-9-4. 진행률이 안 올라가던 문제 (단계 8-3)

특수 상대성 이론에서 **100점을 받아도 진행도가 0%에 머물렀다.**

계산이 틀린 게 아니라 세는 방식이 성겼다. 진행도를 "숙달한 오개념 개수 ÷ 전체"로 계산했는데,

- 숙달하려면 같은 오개념에 정답 **2번**이 필요하고
- 한 문제가 겨냥하는 오개념은 **2개**이고
- 다음 문제는 아직 안 건드린 오개념으로 넘어간다

그래서 오개념 8개짜리 소단원은 **한 바퀴 도는 앞의 4문제 동안 아무도 두 번째 정답을 못 받는다.** 만점 4번이 전부 0%로 보인다.

개수 대신 "각 오개념이 목표에 얼마나 다가갔는지"를 평균 내도록 바꿨다.

| 문제(전부 100점) | 지금 | 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 9 |
|---|---|---|---|---|---|---|---|---|---|---|
| 전 | 0% | 0% | 0% | 0% | 0% | 13% | 38% | 63% | 88% | 100% |
| 후 | 21% | 36% | 53% | 67% | 80% | 89% | 97% | 99% | 99% | 100% |

출발점은 정확히 0%, 전부 숙달하면 정확히 100%, 틀리면 내려간다. **승급 조건은 건드리지 않았다** — 여전히 전부 숙달해야 다음 레벨로 간다. 반올림 때문에 100%인데 승급이 안 되는 경우는 99%에서 막았다.

### 7-9-5. 틀린 문장 개수 강제 + 검수 문항 줄이기 (단계 8-4)

**Level 1에서 틀린 문장이 3개 나왔다.** 요청은 1개였다. 개수는 프롬프트로 지시만 하고 아무도 검사하지 않았다.

이 개수에 걸려 있는 것 — 배점(100 ÷ 틀린 문장 수), 체감 난이도, 헛다리 감점폭. 서버에서 세서 다르면 다시 만들게 했다. 마지막 시도까지 안 맞으면 그대로 쓰고 경고만 남긴다(문제 생성 실패를 띄우는 것보다 낫다).

**검수 문항이 왜 나오는지 짚이는 곳이 있다.** 지난번 사고에서 걸린 문장이 `ε = -N(ΔΦ/Δt)` 였다. 교과서에 따라 크기만 `ε = N(ΔΦ/Δt)`로 쓰기 때문에 **맞다고도 틀리다고도 할 수 있는 문장**이다. 문제를 만든 AI와 채점한 AI의 판정이 갈린 게 당연하다.

공식 판별 문장에서 다음을 금지했다.

| 금지 | 예 |
|---|---|
| 부호 관례만 다른 식 | `ε = -N(ΔΦ/Δt)` |
| 벡터·스칼라 표기 차이 | `F = ma` vs `F⃗ = ma⃗` |
| 기호 이름만 다른 식 | `v` 대신 `u` |
| 조건 없이 제시한 근사식 | 공기 저항 무시 여부로 갈리는 식 |

틀린 공식은 변수가 바뀌거나(`W = mv²`) 분자·분모가 뒤집힌(`a = m/F`) 형태로만 만들게 했다. 표기 관례와 무관하게 틀린 것이어야 한다.

**"검수 중인 문항" 카드를 없앴다.** 문제를 다 푼 학생에게 "이건 저희 사정으로 뺐어요"라고 통보하는 화면이라 보기 좋지 않았다. 검사를 **채점할 때가 아니라 문제를 만들 때**로 옮겼다.

| | 전 | 후 |
|---|---|---|
| 검사 시점 | 학생이 다 푼 뒤 | 학생에게 보내기 전 |
| 어긋나면 | 그 문항을 무효 처리하고 화면에 표시 | 그 세트를 버리고 **다시 만듦** |
| 학생이 보는 것 | "채점에서 뺀 문항" 카드 | 검증을 통과한 문장 5개 |

검증은 **별도 호출**로 한다. 문장만 주고 "물리적으로 거짓인가"만 묻는다. 오개념 목록도, 겨냥 지시도, 생성 당시의 라벨도 주지 않는다 — 힌트를 주면 앞의 판단을 그대로 따라가서 검증이 되지 않는다. 참·거짓을 단정할 수 없는 문장(`ambiguous`)도 같이 버린다.

3번 시도해도 안 맞으면 문제 생성을 실패시키지 않고 검증 쪽 판정을 채택한다. 판정만 하는 호출이, 개수 맞추기·소재 다양화·오개념 겨냥을 겸하는 생성 호출보다 라벨에 관해서는 더 믿을 만하다.

채점할 때의 대조는 **남겨두되 화면에는 쓰지 않는다.** 세 번째 판정이 갈리면 이해도 관측에서만 빼고 로그를 남긴다. 점수 한 문항이 흔들리는 것보다 이해도가 오염되는 쪽이 오래가기 때문이다. 이 로그 건수가 곧 생성 검증을 통과하고도 남은 문항 오류율이라 논문의 실측치로 쓴다.

**남는 비용** — 문제 하나당 AI 호출이 1회 늘어난다.

### 7-9-6. 마이페이지 이해도 표시 (단계 9)

측정한 이해도를 학습자가 볼 수 있는 곳이 없었다. 마이페이지 소단원 상세에 카드 하나를 넣었다.
...
**오개념 하나하나의 값은 안 보여준다.** 처음엔 오개념별 막대를 쭉 나열했는데 걷어냈다. 학습자에게 필요한 건 "이 단원 얼마나 남았나"이지 8줄짜리 표가 아니다. 문제 화면에서 개별 오개념을 감춘 이유가 마이페이지에도 그대로 적용된다.

같은 이유로 메인 통계의 "교정한 오개념 N/M"도 넣었다가 뺐다. 학습 횟수·평균 점수와 달리 그 숫자로 학습자가 할 수 있는 일이 없다.

**두 화면의 숫자를 같은 함수로 통일했다.** `BKT.progressRatio()` 하나를 문제 화면과 마이페이지가 같이 쓴다. 다른 식을 쓰면 같은 상태를 두 화면이 다른 숫자로 말하게 된다.

이해도를 그대로 %로 쓰지 않은 이유 — 아직 한 번도 안 나온 개념이 30%로 표시된다. "30%는 안다"로 읽히지만 실제로는 "아무 증거 없음"이다.

한 번도 측정 안 된 개념도 분모에 넣는다. 측정된 것만 세면 "3개 중 3개 끝냄"과 "8개 중 3개 끝냄"이 구분되지 않는다.

### 7-9-7. 배포해도 옛 파일이 뜨던 문제

배포된 사이트가 `Cache-Control: max-age=3600`으로 응답하고 있었다. 파일 이름에 버전이 안 붙어 있어서, 배포 후 최대 1시간 동안 브라우저가 옛 JS를 그대로 쓴다. 개발 중에는 강제 새로고침으로 넘겼지만 실제 사용자는 그럴 수 없다.

`firebase.json`에 `no-cache`를 넣었다. `no-store`가 아니라 매번 "바뀌었나요?"만 묻는 것이라, 안 바뀌었으면 304로 끝나고 본문을 다시 받지 않는다. `public/`에 이미지도 없어(css·js·html뿐) 실질 비용이 없다.

### 7-9-8. 문항 신고 (단계 9-1)

문장의 참·거짓은 이제 문제를 만들 때 검증한다. 하지만 **계산 문제, 해설, 서술형 채점은 여전히 AI가 혼자 판단**하고 아무도 대조하지 않는다. 그쪽이 틀렸을 때 학생이 말할 곳이 없었다.

결과 화면의 각 문항 카드 아래에 작은 버튼을 넣었다.
...
문제를 푸는 중이 아니라 **결과 화면**인 이유 — 푸는 중에는 답을 모르니 신고할 근거가 없다.

누르면 사유를 고른다. 자유 입력만 받으면 아무도 안 쓰고, 고르기만 시키면 예상 밖의 오류를 못 듣는다. 그래서 고정 항목 5개 뒤에 **기타(직접 입력)**를 뒀다. 기타는 뭐라도 적어야 보내진다.
...
**신고해도 점수와 이해도는 안 바뀐다.** 신고가 관측을 지우게 만들면 "틀리면 신고"가 최적 전략이 되어 이해도가 영원히 안 떨어진다. 학생에게는 "확인해볼게요"만 뜨고, 모아서 사람이 고친다.

저장은 `question_reports`에 하고, 문장 원문·해설·학생 답변을 통째로 복사해 넣는다. 세션 참조만 남기면 문제를 고쳐 다시 시딩했을 때 원래 뭐가 문제였는지 사라진다.

Firestore 규칙은 **만들기만 허용**한다. 읽기를 막는 건 신고에 그 사람 답변이 들어가기 때문이고, 수정·삭제를 막는 건 논문 집계에 쓰기 때문이다. 검토는 `node functions/list-reports.js`로 한다.

논문에서는 신고율(체감 오류율), 사유 분포, 그리고 **자동 검출과 겹치는 비율**을 쓴다. 마지막이 핵심이다 — 지금 자동 검출에는 정답지가 없는데, 학생 신고가 그 역할을 한다.
...
| `firestore.rules` | 판별 문장·개념 영역 컬렉션 읽기 허용, 문항 신고 생성만 허용 |
| `functions/list-reports.js` | 신규. 문항 신고 조회·집계 (관리자 SDK) |
| `public/firebase/firestore.js` | knowledgeState CRUD, 관측 반영, 레벨 대상 선정, 승급 판정, 진행률 계산, 이해도 리셋, 진단검사 조회·저장 |
| `public/js/feedback.js` | 승급 판정 교체, 재도전 제외, 진행도 표시, 검수 문항 분리 |
| `public/js/app.js` | 순환 출제 헬퍼, 단원 정보 모달, 나가기 목적지, 진단 화면 라우팅 |
| `public/js/keyword.js` | 진단 오개념 누적, 순환 출제 연결, 진단검사 진입 |
| `public/js/mypage.js` | 진단 풀 사용, 순환 출제 연결, 진단검사 진입, 소단원 이해도 표시 |
| `public/js/quiz.js` | 문항 태그를 채점 결과에 전달 |
| `functions/index.js` | 문항 오개념 태깅, 우선 겨냥 지시, 사진 진단 정리, 문항 검수, 미체크 문항 처리 확정 |
| `functions/seed.js` | 오개념 87 → 128개, 판별 문장 166 → 342개, 전수 검증 반영, 시딩 시 폐기 문서 삭제 |
| `firebase.json` | 서비스 계정 키·로컬 스크립트를 함수 배포에서 제외(`functions.ignore`), 정적 파일 `no-cache` |
| `functions/reset-knowledge.js` | 신규. 오염된 이해도 초기화 (미리보기 기본) |
| `public/index.html` | 단원 정보 모달, 문제 화면 상단 버튼, 진단검사 화면 |
| `public/css/base.css`, `feedback.css`, `quiz.css` | 정보 버튼, 진행도 막대, 텍스트 버튼, 진단검사 화면 |

### 7-11. 남은 것

- **파라미터 민감도 분석(단계 10, 논문 실험)** — τ와 P(L₀)는 "이렇게 정했다"만 있고 "이 값이 아니면 어떻게 되는가"가 없다 (최우선)
- Level 1 문장을 검증된 판별 문장 풀에서 가져오기 — 단계 8-5 검증이 들어가 급하지 않다. 신고·검출 데이터를 보고 결정
