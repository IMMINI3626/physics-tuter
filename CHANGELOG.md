...
측정한 이해도를 학습자가 볼 수 있는 곳이 없었다. 문제 화면에는 소단원 진행도 하나만 띄우기로 했기 때문에(풀이 중에 개별 오개념이 보이면 학습보다 숫자를 좇게 된다), 마이페이지가 유일한 자리다.

| 위치 | 추가한 것 |
|---|---|
| 마이페이지 메인 | 통계 카드 "교정한 오개념 N/M" |
| 소단원 상세 | "개념별 이해도" 카드 — 소단원 전체 % + 오개념별 막대 |

오개념별 목록은 **이해도가 낮은 것부터** 보여준다. 확인이 끝난 개념은 초록 체크로 접힌다.

**두 화면의 숫자를 같은 함수로 통일했다.** `BKT.progressRatio()` 하나를 문제 화면과 마이페이지가 같이 쓴다. 소단원 상세의 오개념별 값 평균이 문제 화면 진행도와 정확히 같다. 다른 식을 쓰면 같은 상태를 두 화면이 다른 숫자로 말하게 된다.

이해도를 그대로 %로 쓰지 않은 이유 — 아직 한 번도 안 나온 개념이 30%로 표시된다. "30%는 안다"로 읽히지만 실제로는 "아무 증거 없음"이다.

"교정한 오개념"의 분모는 **측정을 시작한 개수**다. 전체 128개를 분모로 쓰면 누구든 한 자릿수 %라 아무것도 알려주지 못한다.

한 번도 측정 안 된 개념도 목록에 0%로 넣는다. 측정된 것만 보이면 "3개 중 3개 끝냄"과 "8개 중 3개 끝냄"이 구분되지 않는다.
...
| `public/js/mypage.js` | 진단 풀 사용, 순환 출제 연결, 진단검사 진입, 개념별 이해도 표시 |
