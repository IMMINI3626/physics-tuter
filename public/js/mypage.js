const MypageScreen = {
  ...
  async retryUnit(btnEl) {
    ...
    try {
      ...
      const targets = await pickTargetMisconceptionIds(this._currentSubUnit, level);
      const result = await ApiService.generateQuestions([], this._currentSubUnit, level, mode, targets);
